 ...
 * @author EMAIL:zhang1kr@gmail.com Update
 *
 */
public class CrawlData implements Serializable {
    ...
    public CrawlData(String url, List<Position> positionList) {
    	this.url = url;
    	this.positionList = positionList;
    }
    ...
    public CrawlData setTextValue(String textValue, WebPage page) {
        ...
        injectDataToPageMetadata(code, textValue == null ? null : textValue.toString(), page);
        return this;
    }
    ...
    public CrawlData setNumValue(String numValue, WebPage page) {
        ...
        try {
            this.numValue = StringUtils.isBlank(numValue) ? null : new BigDecimal(numValue);
        } catch (Exception e) {
            LOG.warn("Parse String to BigDecimal error, num value is " + numValue);
        }
	...
	private void injectDataToPageMetadata(String code, String value, WebPage page) {
        if (page != null) {
            // 添加固定前缀标示属性需要添加到Solr Index属性中
            ...
            if (value == null) {
                ...
                page.getMetadata().put(new Utf8(metadataCode), ByteBuffer.wrap(Bytes.toBytes(value)));
            }
        }
    }
